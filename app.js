...
//Endpoint para verificar todas os funcionarios
app.get('/v1/dream-chock/api/all-employee/rh', cors(), async function(request, response){


    let resultAllEmployee = await controllerDreamChockRh.getAllEmployees()

    response.status(resultAllEmployee.status)
    response.json(resultAllEmployee)
